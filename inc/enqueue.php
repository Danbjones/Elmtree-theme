...
if ( ! function_exists( 'custom_scripts' ) ) {
	...
	function custom_scripts() {
		...
		if ($current_env === 'production' && file_exists(get_template_directory() . '/dist/js/bundle.js')) {
			...
			wp_enqueue_script( 'custom-scripts', get_template_directory_uri() . '/dist/js/bundle.js', array('jquery'), $js_version, true );
		}

		if ($current_env === 'development') {
			wp_enqueue_script( 'custom-scripts', 'http://localhost:3000/js/bundle.js', array('jquery'), false, true );
		}
	}
}
